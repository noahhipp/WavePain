...
mean_func_dir       = 'run001/mrt/';
skullstrip_name     = 'skull_strip.nii';
dup_mean            = 'mean_epi';
struct_dir          = 'run000/mrt/';
...
for np = 1:size(subs,2)
    ...
    for g = 1:size(subs{np},2)
        ...
        strip_file = [st_dir skullstrip_name];
        ...
        if do_norm
            ...
            mbi = mbi + 1;                          
            mean_file   = [st_dir mean_func_templ];
            
            matlabbatch{mbi}.spm.tools.dartel.crt_warped.flowfields = cellstr(u_rc1_file);
            matlabbatch{mbi}.spm.tools.dartel.crt_warped.images = {cellstr(mean_file)};
