...
% Housekeeping
eda_name_in     = 'online_native_scl_behav.mat';
...
% M W], page->ID.
load(eda_file_in, 'native_scl_online', 'subjects');
online_ratings = readtable(online_rating_file);
overall_ratings = readtable(overall_rating_file);

% To make it run on aranyani
onlineScl = native_scl_online;
...
data.native_scl            = reshape(onlineScl, [],1);
