...
if ~exist(EDA_BINNED_DIFF_FILE, 'file')
    ...
    S.m10.bsd = S.m10.(DIFF_VAR) - S.m20.(DIFF_VAR);
    S.m20.bsd = S.m10.(DIFF_VAR) - S.m20.(DIFF_VAR);
    S.m10.bsq = S.m10.(DIFF_VAR) < S.m20.(DIFF_VAR); % when we mean this later we get the desired ratio
    S.m20.bsq = S.m10.(DIFF_VAR) < S.m20.(DIFF_VAR); % this is redundant but better safe than sorry
    
    % Second M segment
    S.m01 = d(d.wm == -1 & d.slope ==  1 & d.condition == 1,:); %m21 1back
    S.m02 = d(d.wm ==  1 & d.slope ==  1 & d.condition == 2,:);
    S.m01.bsd = S.m01.(DIFF_VAR) - S.m02.(DIFF_VAR); % negative values indicate 1back < 2back
    S.m02.bsd = S.m01.(DIFF_VAR) - S.m02.(DIFF_VAR);
    S.m01.bsq = S.m01.(DIFF_VAR) < S.m02.(DIFF_VAR); % values close to 1 indicate 1back < 2back
    S.m02.bsq = S.m01.(DIFF_VAR) < S.m02.(DIFF_VAR);
    
    % First W segment
    S.w20 = d(d.wm ==  1 & d.slope ==  1 & d.condition == 3,:);
    S.w10 = d(d.wm == -1 & d.slope ==  1 & d.condition == 4,:);
    S.w10.bsd = S.w10.(DIFF_VAR) - S.w20.(DIFF_VAR);
    S.w20.bsd = S.w10.(DIFF_VAR) - S.w20.(DIFF_VAR);
    S.w10.bsq = S.w10.(DIFF_VAR) < S.w20.(DIFF_VAR);
    S.w20.bsq = S.w10.(DIFF_VAR) < S.w20.(DIFF_VAR);
    
    % Second W segment
    S.w02 = d(d.wm ==  1 & d.slope == -1 & d.condition == 4,:);
    S.w01 = d(d.wm == -1 & d.slope == -1 & d.condition == 3,:);
    S.w01.bsd = S.w01.(DIFF_VAR) - S.w02.(DIFF_VAR);
    S.w02.bsd = S.w01.(DIFF_VAR) - S.w02.(DIFF_VAR);
    S.w01.bsq = S.w01.(DIFF_VAR) < S.w02.(DIFF_VAR);
    S.w02.bsq = S.w01.(DIFF_VAR) < S.w02.(DIFF_VAR);
    ...
    if isequal(d_up.condition, d_down.condition)
        ...
        % Now we load the OPR FILE and check again to be double safe and
        ...
        if isequal(d_up.condition, d_opr.condition)
            ...
            % Looks good we stick usd and usq to d_down 
            d_down.usd = d_up.usd;
            d_down.usq = d_up.usq;
            
            % and the OPRs we can hopefully explain with dsd, dsq, usd, usq
            d_down.rating = d_opr.rating;                        
            
            % Write output
            writetable(d_down, EDA_BINNED_DIFF_FILE);
